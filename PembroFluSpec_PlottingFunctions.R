...
twoSampleBar <- function (data, xData, yData, fillParam, title, yLabel, batch="none", position = "left", FCplot=F, confInt=F, ttest=T, nonparam=F)
{
  pValue = 0
  set.seed(102)
  if (ttest == T && batch == "none" && nonparam == F)  # path 1
  {
    print("path 1:  ttest == T && batch == none && nonparam == F")
  ...
  if (ttest == T && batch == "none" && nonparam == T)   # path 2
  {
    print("path 2:  ttest == T && batch == none && nonparam == T")
    justforttest <- data[, c(xData,yData)]
  ...
  if (! is.nan(pValue) && confInt == T)   # path 4
  {
    print("path 4:  ! is.nan(pValue) && confInt == T")
    if (pValue < 0.01)
      {    annotationInfo <- paste0("P = ", formatC(pValue, format="e",digits=1), "\n", "Mean 95%CI: [",CI[1], ", ",CI[2], "]")     }
    if (pValue >= 0.01)
    {    annotationInfo <- paste0("P = ", round(pValue, 2),"\n", "Mean 95%CI: \n[",CI[1], ", ",CI[2], "]")      }
  ...
  if (! is.nan(pValue) && confInt == F) # path 5
  {
    print("path 5:  ! is.nan(pValue) && confInt == F")
    if (pValue < 0.01)
    {    annotationInfo <- paste0("P = ", formatC(pValue, format="e",digits=1))     }
    if (pValue >= 0.01)
    {    annotationInfo <- paste0("P = ", round(pValue, 2))      }
    if(position == "left")  { my_grob = grobTree(textGrob(annotationInfo, x=0.03,  y=0.93, hjust=0, gp=gpar(col="black", fontsize=28)))   }
    if(position == "right")  { my_grob = grobTree(textGrob(annotationInfo, x=0.45,  y=0.93, hjust=0, gp=gpar(col="black", fontsize=28)))   }
    if(position == "none") { my_grob = grobTree(textGrob(annotationInfo, x=10,  y=10, hjust=0, gp=gpar(col="black", fontsize=1)))   }
  ...
  return (
    ggplot(data=data, aes_string(x=xData, y=yData, fill=fillParam, width=0.6)) + scale_fill_manual(values = c("#7FAEDB", "#FFB18C")) + 
      ...
      ggbeeswarm::geom_quasirandom(size=7,pch=21,fill="black",color="white",alpha=0.5, varwidth = T)+ 
      ggtitle(title) + ylab(yLabel) +  theme_bw() +
...
univScatter <- function(data, xData, yData, fillParam, title, xLabel, yLabel, position = "left", statsOff=F, nonparam=F)
{
  if (statsOff == T)
  {
    return (
  ...
  if (nonparam==T)
  {
    kendall <- round(cor(data[,xData], data[,yData], method = "kendall", use = "complete.obs"), 2)
    pValue <- cor.test(data[,xData], data[,yData], method="kendall")
    if (pValue$p.value < 0.01)   {    annotationInfo <- paste0("Kendall t = ", kendall,"\n","P = ", formatC(pValue$p.value, format="e", digits=1))    }
...
bivScatter <- function(data1, data2, name1, name2, xData, yData, fillParam, title, xLabel, yLabel, statsOff=F, nonparam=F)
{
  if (statsOff == T)
  {  
    return (
  ...
  {  
    tau1 <- round(cor(data1[,xData], data1[,yData], method = "kendall", use = "complete.obs"), 2)
    pValue1 <- cor.test(data1[,xData], data1[,yData], method="kendall")
    tau2 <- round(cor(data2[,xData], data2[,yData], method = "kendall", use = "complete.obs"), 2)
    pValue2 <- cor.test(data2[,xData], data2[,yData], method="kendall")
    if (pValue1$p.value < 0.01 | pValue2$p.value < 0.01)   {    
      annotationInfo1 <- paste0(name1, " Kendall t = ", tau1,"\n","P = ", formatC(pValue1$p.value, format="e", digits=1) )
      annotationInfo2 <- paste0("\n", name2, " Kendall t = ", tau2,"\n","P = ", formatC(pValue2$p.value, format="e", digits=1) )  }
    if (pValue1$p.value >= 0.01 & pValue2$p.value >= 0.01) {    
      annotationInfo1 <- paste0(name1, " Kendall t = ", tau1,"\n","P = ", round(pValue1$p.value,2) )
      annotationInfo2 <- paste0("\n",name2," Kendall t = ", tau2, "\n","P = ", round(pValue2$p.value,2))   }
    my_grob1 = grobTree(textGrob(annotationInfo1, x=0.05,  y=0.88, hjust=0, gp=gpar(col="#7FAEDB", fontsize=28)))
...
prePostTimeAveraged <- function(data, title, xLabel, yLabel)
{
  ...
  my_grob2 = grobTree(textGrob(annotationInfo2, x=0.05,  y=0.85, hjust=0, gp=gpar(col="#7FAEDB", fontsize=28)))
